...
exports.handleCommand = async (client, message) => {
    ...
    if (text === '4') {
        ...
        await client.sendMessage(from, '❌ Atendimento cancelado. Se quiser voltar, digite *oi*.');
        return;
    }
    if (text === 'finalizar') {
    ...
    await client.sendMessage(from, '❌ Comando não reconhecido. Por favor, digite *oi* para ver as opções.');
};